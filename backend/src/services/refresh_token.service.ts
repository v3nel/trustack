...
export async function getRefreshToken(user_id: number) {
    return db.select().from(refresh_token).where(eq(refresh_token.user_id, user_id)).orderBy(refresh_token.created_at).limit(1)
}

export async function revokeRefreshToken(id: number) {
    const result = await db.update(refresh_token)
